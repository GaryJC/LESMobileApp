...
class MessageCaches {
    ...
    static MakeChatIDByMsgData(msgData) {
        if (msgData.messageType == LesConstants.IMMessageType.Group) {
            return MessageCaches.MakeChatID(msgData.senderId, msgData.groupId, msgData.messageType);
        } else {
            return MessageCaches.MakeChatID(msgData.senderId, msgData.recipentId, msgData.messageType);
        }
    }

    static MakeChatID(senderId, recipentId, type = LesConstants.IMMessageType.Single) {
    ...
    #getChatData(msgData) {
        ...
        if (chatData == null) {
            let targetId = msgData.senderId == this.#currUserId ? msgData.recipentId : msgData.senderId;
            if (msgData.messageType == IMMessageType.Group) {
                targetId = msgData.groupId;
            }

            chatData = this.#chatDatMap[chatId] = new ChatData(chatId, msgData.messageType, targetId);
        }
        //同步建立dataSorted
    ...
    pushMessage(msgData) {
        ...
        if (chatData.addOrUpdate(msgData)) {
            ...
            chatSort.gotNewMessage(msgData.content, msgData.senderId != this.#currUserId);
            this.#sortChatList();
        }
        return msgData;
    }
...
class ChatData {
     ...
     */
    addOrUpdate(msgData) {
        const idx = this.#messageList.findIndex(item => item.messageId == msgData.messageId);
        let isNew = false;
        if (idx == -1) {
