...
import { View, Text, FlatList, RefreshControl, ActivityIndicator, Animated } from "react-native";
...
const MessageTitle = ({ chatObj }) => {
    ...
    const [target, setTarget] = useState({ id: 0, name: "", type: IMMessageType.Single, data: null });

    const nav = useNavigation();

    const setGroupTarget = (cg) => {
        if (cg != null) {
            const tn = { id: cg.id, name: cg.name, type: IMMessageType.Group, data: cg };
            setTarget(tn);

            ChatGroupService.Inst.getGroupMembers(cg.id, state => { return state == IMGroupMemberState.Confirmed })
                .then(members => {
                    //if (target.id == cg.id) {
                    const nt = { ...tn };
                    nt.members = members;
    ...
    const setUserTarget = (user) => {
        if (user != null) {
            const tn = { id: user.id, name: user.name, type: IMMessageType.Single, data: user };
            setTarget(tn);
        }
    }
...
const MessagePanel = ({ chatData, focusMessaageId }) => {
    ...
    const loadMessage = (chatData, init = false) => {
        ...
        const loaded = { from: init ? from : messageLoaded.from, loaded: init ? msgs.length : messageLoaded.loaded + msgs.length }
    ...
    const msgListener = ({ chatId, msgData }) => {
        ...
        if (isExisted) {
            ...
            const loaded = { from: messageLoaded.from, loaded: messageLoaded.loaded + 1 }
            setMessageLoaded(loaded);
        }
    };
    ...
    useEffect(() => {
        if (scrollToMsgId != null) {
            const idx = messagesRef.current.findIndex(msg => msg.messageId == scrollToMsgId);
            if (idx >= 0) {
                flatListRef.current.scrollToIndex({ animated: false, index: idx, viewPosition: 0 });
            }
        }
