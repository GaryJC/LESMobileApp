...
const HighlightButton = ({ type, text, isLoading, disabled, onPress }) => {
    ...
    const bgClassName = `bg-[${_clr.bg}] px-[10px] py-[5px] rounded flex flex-row  ${isLoading ? "opacity-50" : ""}`
    const fgClassName = `text-[${_clr.fg}] text-center`

    return <TouchableHighlight onPress={disabled ? null : onPressHandler} className="mx-[5px] rounded">
