...
export default NotificationDetailScreen = ({ route, navigation }) => {
  ...
  return (
    <View className="m-2 mt-5 h-full">
      <View className="flex flex-row ">
        <View>
          <Image
            source={Constants.Icons.getSystemIcon(notification.sender.id)}
        ...
        <View className="flex ml-[10px] flex-1">
          ...
          <View className="flex flex-row items-center">
            <Text className="text-base text-white font-bold pr-1">
        ...
        <View className="flex">
          ...
          <HighlightButton
            ...
            onPress={() => {
              setLoading(true);
              NotificationService.Inst.deleteSystemNotification(
                notification.id
              ).then((id) => {
                setLoading(false);
                navigation.goBack();
      ...
      <View className="flex w-full bg-black mb-2 rounded-xl">
        <Text className="text-white text-lg font-bold py-2 text-center">
...
const contentToDom = (content) => {
  switch (content.type) {
    ...
    case "image":
      const img = (
        <ScaledImage uri={content.imageUrl} width={screenWidth - 20} />
      );
      if (content.link && content.link.length > 0) {
        console.log("===", content);
        return (
          <TouchableHighlight onPress={() => Linking.openURL(content.link)}>
            {img}
          </TouchableHighlight>
        );
      } else {
      ...
      return (
        <View className="my-1">
          <HighlightButton
            icon={
              <Image
                source={Constants.Icons.getSystemIcon(content.imageUrl)}
  ...
  switch (type) {
    case "image":
      ...
      if (imgResult.length >= 2) {
        ...
        console.error(
          "image tag format error expecting[!image src= link=]: " + tag
        );
    ...
    case "button":
      ...
      if (btnResult.length == 4) {
        ...
        console.error(
          "button tag format error expecting[!button title=  link=  image=]: " +
            tag
        );
        return newTextContent("");
      }
